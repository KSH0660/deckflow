...
from tests.builders import any_deck_plan, any_slide_content, DeckPlanBuilder
        ...
        with patch('app.api.deck.current_repo') as mock_repo_factory:
            ...
            mock_repo.get_deck.return_value = {
                ...
                "slides": [
                    {
                        "order": 1,
                        "content": {"html_content": "<div>Test slide</div>"}
                    }
                ]
            }
...
class TestAPIErrorHandling:
    ...
    def test_modify_non_completed_deck(self, client):
        ...
        with patch('app.api.deck.current_repo') as mock_repo_factory:
            ...
            assert "Can only modify slides in completed decks" in response.json()["detail"]
