...
class TestWriteContent:
    ...
    async def test_write_content_missing_params(self, mock_llm):
        ...
        with pytest.raises(ValueError, match="슬라이드 정보와 덱 컨텍스트는 필수입니다"):
    ...
    @pytest.mark.asyncio
    async def test_write_content_modification_mode(self, mock_llm, sample_slide_content):
    ...
    @pytest.mark.asyncio
    async def test_write_content_prompt_formatting(self, mock_llm, sample_slide_content):
...
class TestValidateSlideContent:
    ...
    def test_validate_slide_content_overflow_checks(self, caplog):
        ...
        bad_content = SlideContent(html_content="""
        <!DOCTYPE html>
        <html>
...
class TestRenderPrompt:
    ...
    def test_render_prompt_overflow_prevention(self):
        ...
        assert "text-3xl or larger" in RENDER_PROMPT or "text-2xl maximum" in RENDER_PROMPT