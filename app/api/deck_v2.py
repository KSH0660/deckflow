...
from app.core.config import Settings as AppSettings, settings as app_settings
...
router = APIRouter(tags=["decks-v2"], prefix="/v2")
...
def get_deck_service(
    repo=Depends(current_repo),
    llm=Depends(current_llm)
) -> DeckService:
...
async def create_deck(
    ...
    try:
        ...
        response = await deck_service.create_deck(request)
        
        # Start background generation (existing logic)
        deck_uuid = UUID(response.deck_id)
        repo = current_repo()
...
@router.get("/decks/{deck_id}/slides/{slide_order}/versions", response_model=SlideVersionHistoryResponse)
...
@router.post("/decks/{deck_id}/slides/{slide_order}/revert", response_model=RevertSlideResponse)
async def revert_slide_to_version(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"버전 되돌리기 실패: {str(e)}")
...
async def save_edited_html(
    ...
    try:
        ...
        response = await deck_service.save_slide_content(deck_uuid, slide_order, html_str)
