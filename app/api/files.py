...
async def upload_file(file: UploadFile = File(...)):
    ...
    logger.info("📤 [FILE_UPLOAD] 파일 업로드 요청 시작", filename=file.filename, content_type=file.content_type)
    ...
    if not FileProcessor.is_allowed_file(file.filename):
        logger.warning("📤 [FILE_UPLOAD] 지원하지 않는 파일 형식", 
    ...
    logger.debug("📤 [FILE_UPLOAD] 파일 읽기 완료", 
                filename=file.filename, 
                file_size_bytes=file_size,
                file_size_kb=round(file_size / 1024, 2))
    
    if not FileProcessor.is_valid_size(file_size):
        logger.warning("📤 [FILE_UPLOAD] 파일 크기 초과", 
                      filename=file.filename,
    ...
    try:
        ...
        logger.info("✅ [FILE_UPLOAD] 파일 업로드 및 처리 완료", 
                   filename=file.filename,
                   file_size_kb=round(file_size / 1024, 2),
        ...
        logger.error("📤 [FILE_UPLOAD] 파일 검증 오류", 
                    filename=file.filename, error=str(e))
        raise HTTPException(status_code=400, detail=str(e)) from e
    except Exception as e:
        logger.error("📤 [FILE_UPLOAD] 파일 처리 중 예상치 못한 오류", 
                    filename=file.filename, error=str(e))
        raise HTTPException(status_code=500, detail=f"파일 처리 중 오류가 발생했습니다: {str(e)}") from e
...
async def validate_file(file: UploadFile = File(...)):
    ...
    if not file.filename:
        return JSONResponse(
            status_code=400,
            content={"valid": False, "message": "파일명이 필요합니다."}
        )
