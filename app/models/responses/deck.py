...
class ModifySlideResponse(BaseModel):
    ...
    status: str = Field(
        default="modifying", description="Current status of the modification"
    )
...
class SaveSlideContentResponse(BaseModel):
    ...
    version_id: str | None = Field(None, description="ID of the new version created")
    version_count: int = Field(
        ..., description="Total number of versions for this slide"
    )
