        ...
        for file_name in selected_files:
            ...
            try:
                with open(file_path, encoding='utf-8') as f:
                    ...
                    logger.debug("템플릿 로드 성공", file_name=file_name, content_length=len(content))
            except Exception as e:
                logger.warning("템플릿 파일 읽기 실패", file_name=file_name, error=str(e))
                continue

        logger.info("템플릿 예제 로드 완료",
...
async def write_content(slide_info: dict, deck_context: dict, llm) -> SlideContent:
    ...
    mapped_color_theme = COLOR_THEME_MAPPING.get(color_theme, COLOR_THEME_MAPPING['professional_blue'])

    logger.info("슬라이드 콘텐츠 생성 시작",
                number=slide_number,
    ...
    try:
        ...
        template_examples_text = "\n".join(template_examples) if template_examples else "No template examples available for this layout type."
...
def _validate_slide_content(content: SlideContent, slide_info: dict, deck_context: dict) -> None:
    ...
    if missing_elements:
        logger.warning("슬라이드에 필수 요소 누락 발견",
                      slide_title=slide_title,
                      missing_elements=missing_elements)

    # HTML 길이 검증
    if len(html) < 500:
        logger.warning("슬라이드 HTML이 너무 짧음",
                      slide_title=slide_title,
                      html_length=len(html))
    elif len(html) > 10000:
        logger.warning("슬라이드 HTML이 너무 긺",
                      slide_title=slide_title,
                      html_length=len(html))
    ...
    has_colors = any(indicator in html for indicator in [
        'bg-', 'text-', 'border-', 'from-', 'to-', '#', 'rgb', 'gradient'
    ])

    if not has_colors:
        logger.warning("색상 스타일링이 감지되지 않음", slide_title=slide_title)
    else:
        logger.debug("색상 스타일링 확인됨", slide_title=slide_title, color_theme=color_theme)

    logger.debug("슬라이드 콘텐츠 검증 완료",
                slide_title=slide_title,
                html_length=len(html),
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        try:
            ...
            logger.info(f"🚀 생성 속도: {len(content.html_content)/content_generation_time:.0f}자/초")

            # 캐싱 정보 확인 (LLM이 빠르게 응답했다면 캐시 히트 가능성)
            cache_performance = "🚀 캐시 히트 가능성 높음" if content_generation_time < 1.0 else "🐌 새로운 생성으로 추정"
            logger.info(f"🔄 캐시 상태: {cache_performance}")

            # 템플릿 로드 테스트
            template_start = time.time()
            test_templates = _get_template_examples(LayoutType.FEATURE_SHOWCASE.value, max_templates=2)
            template_end = time.time()
            template_load_time = template_end - template_start

            logger.info(f"📂 템플릿 로드: {len(test_templates)}개, 시간: {template_load_time:.3f}초")
            ...
            required_checks = [
                ...
                ("색상 스타일링", any(indicator in content.html_content for indicator in ['bg-', 'text-', '#'])),
                ("다크 테마", any(dark in content.html_content for dark in ["bg-gray-900", "bg-black", "text-white"])),
            ]
            ...
            logger.info(f"📊 품질 점수: {passed_count}/{len(required_checks)} ({passed_count/len(required_checks)*100:.1f}%)")
            logger.info(f"⏱️  검증 시간: {validation_time:.3f}초")

            # 생성된 HTML의 미리보기
            preview = content.html_content[:300] + "..." if len(content.html_content) > 300 else content.html_content
            ...
            # 시간 분석
            logger.info("=== 시간 분석 ===")
            logger.info(f"  콘텐츠 생성: {content_generation_time:.2f}초 ({content_generation_time/total_time*100:.1f}%)")
            logger.info(f"  템플릿 로드: {template_load_time:.3f}초 ({template_load_time/total_time*100:.1f}%)")
            logger.info(f"  품질 검증: {validation_time:.3f}초 ({validation_time/total_time*100:.1f}%)")
            logger.info(f"  파일 저장: {file_save_time:.3f}초 ({file_save_time/total_time*100:.1f}%)")
