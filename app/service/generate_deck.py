...
if __name__ == "__main__":
    ...
    class MockRepository:
        ...
        async def save_deck(self, deck_id, deck_data):
            ...
            logger.debug("덱 저장 완료", deck_id=str(deck_id), status=deck_data.get('status'))
    ...
    async def main():
        ...
        try:
            ...
            if generated_deck:
                ...
                logger.info(f"💬 핵심 메시지: {generated_deck.get('core_message', 'N/A')[:100]}...")
                logger.info(f"📊 생성된 슬라이드 수: {slide_count}개")
                logger.info(f"⏱️  총 생성 시간: {deck_generation_time:.2f}초")
                logger.info(f"🚀 슬라이드당 평균 시간: {deck_generation_time/slide_count:.2f}초" if slide_count > 0 else "🚀 슬라이드당 평균 시간: N/A")
                ...
                for i, slide in enumerate(slides):
                    ...
                    html_length = len(slide_content.get('html_content', '')) if slide_content else 0
                    logger.info(f"  {i+1}. {slide.get('order', 'unknown')} "
                ...
                logger.info(f"  생성 속도: {total_html_chars/deck_generation_time:.0f}자/초")
                logger.info(f"  평균 슬라이드 크기: {total_html_chars/slide_count:.0f}자" if slide_count > 0 else "  평균 슬라이드 크기: N/A")

                # 품질 지표
                successful_slides = len([s for s in slides if s.get('content', {}).get('html_content')])
                success_rate = successful_slides / slide_count * 100 if slide_count > 0 else 0

                logger.info("=== 📊 품질 지표 ===")
                logger.info(f"  성공률: {successful_slides}/{slide_count} ({success_rate:.1f}%)")
                ...
                logger.info(f"  덱 플래닝: ~{estimated_planning_time:.2f}초 ({estimated_planning_time/deck_generation_time*100:.1f}%)")
                logger.info(f"  콘텐츠 생성: ~{estimated_content_time:.2f}초 ({estimated_content_time/deck_generation_time*100:.1f}%)")
